...
export const buildE = ({nName="div", ...opts}={}) => {
  ...
  if (opts.innerHTML) {  
    node.innerHTML = typeof(opts.innerHTML) == "string" ? opts.innetHTML : opts.innerHTML.filter(e => e).join("")
  }
...
export const showBanner = ({message, heading, klass, ...opts}={}) => {
  ...
  const nHeading = heading ? buildE({nName: "h1", textContent: heading}) : null
  const nContent = message ? buildE({textContent: message}) : null
  const btn = opts.noBtn ? null :
    buildE({nName: "button", textContent: "Dismiss", className: "btn", click: () => {
