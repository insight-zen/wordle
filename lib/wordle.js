...
class Game  {
  ...
  savePlay({result, ...opts}={}) {
    ...
    const progArr = localStorage.getItem(LS_STATS_KEY) || []
  ...
  stats(){
    ...
    const message = JSON.stringify(rv)
    showAlert({message, duration: null, btnArr})
  }
...
// Hide menus
const hideMenus = () => document.querySelectorAll("[data-r='menu']").forEach(d => d.classList.toggle("hidden"))
...
export const Wordle = {
  ...
  start(noBind, target){
    ...
    if (!noBind) Targets.replayButton.addEventListener("click", (event) => replayCB("abort"))
  ...
  checkWinLoose(guess, tiles) {
	...
	duration: null, className: "go", btnArr
      })
      danceTiles(tiles)
      stopInteraction()
      return
