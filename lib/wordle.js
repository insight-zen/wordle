...
class Stats {
  ...
  message(){
    if (!this.plays || this.numPlays == 0) return `Stats available after playing a game`
...
class Game  {
  ...
  letterSuggest(){
    ...
    const histo = refDict.histogram({wlist: refDict.filter({check: "hasAll", letters, pattern}), format: "pct"})
...
Can be eliminated: ${this.notIn.join(",")}`
  ...
  screenStr(){
    return `W: ${window.innerWidth}, H: ${window.innerHeight}, PR: ${window.devicePixelRatio}`
  }
  ...
  helpStr(){
    ...
    <div>${strMoves()}</div>
    <div><h4>Screen</h4>${this.screenStr()}</div>
    `
    return msgStr
  } 
