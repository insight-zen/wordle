...
const Wordle = {
  ...
  checkWinLoose(guess, tiles) {
    ...
    if (remainingTiles.length === 0) {
      this.showAlert(`The word was ${targetWord.toUpperCase()}`, null)
      this.stopInteraction()
    }
  ...
  get status(){
    const str = `Moves: ${this.moves.length}, Placed: ${JSON.stringify(this.placed)}, Picked: ${this.picked}, Eliminated: ${this.eliminated}`
    // console.log(str)
  ...
  dictMatchOptions(){
    const rv = { }
    if (Object.keys(this.placed).length > 0 ){
      rv.matches = [0,1,2,3,4].reduce((acc, ix) => `${acc}${wl.placed[ix] || '?'}`, "")
    }
