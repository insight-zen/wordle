...
export class WordList {
  ...
  // bucketFunction is called with a word and expects a bucket to which the word belongs
  // To bucket by string length: (word) => word.length
  // # vowels: (word) => { const matches = word.match(/[aeiou]/ig); return matches ? matches.length : 0 }
  histoBuild(bucketFunc){
    let rv = {}, bucket = null
    this.words.forEach(word => {
      bucket = bucketFunc(word)
      rv[bucket] = (rv[bucket] || 0) + 1
    })
    return(rv)
  }

  // Run the function on each word and return all words where func returns true
  // words with no vowels: wDict.filterFun((word) => wDict.vowelCount(word) == 0 )
  filterFun(func){
    return this.words.filter(func)
  }
