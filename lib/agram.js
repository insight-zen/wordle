...
const Board = {
  ...
  setup(){
    ...
    // Board.divRack.style = `${Board.divRack.attributesstyle}; --ncols: ${WORD_LENGTH};`
    
    // const strBtn = `<button class="key"></button>` // Rack tiles are button. You can use click event here
    const strTile = `<div class="tile"></div>`
    // Board.divRack.insertAdjacentHTML("beforeend", Array.from({length: WORD_LENGTH}).map((e, ix) => strBtn).join(""))
    Board.divAttempt.insertAdjacentHTML("beforeend", Array.from({length: WORD_LENGTH * GUESS_ROWS}).map((e, ix) => strTile).join(""))
    AG.getGuess()
    // const rackButtons = Board.divRack.querySelectorAll("button.key")
    // AG.game.tiles.forEach((c, ix) => rackButtons[ix].textContent = c)
    Board.setupRack()
